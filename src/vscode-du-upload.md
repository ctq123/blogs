...
另外一点需要注意的是，新的文件名的生成会出现长度不一的情况，比如上传媒体类型的数据，新生成的文件名会相对较长，这里采用摘要命名法，永远保持一样的名字长度。
...
同时markdown本身是支持媒体显示的，但vscode.MarkdownString暂时不支持媒体类渲染，查询了官网issue也有人遇到过相同的类型，暂时还没有解决。这里有试过加html之类的标志，但最终都不行，如果有童鞋知道解决方案，欢迎告知～
...
最近在思考工程资源相关的问题，但又不会做成像飞冰那么浩大的工程，毕竟人力有限～
